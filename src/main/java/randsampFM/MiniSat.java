...
public class MiniSat {// implements SatFactory, Dimacs {
  ...
  /**
   * A call to this method will attempt to count the number of solutions of the model
   *
  ...
  BigInteger countSearch() {
    ...
    for (; ; ) {
      ...
      if (confl != CR_Undef) {
        ...
        conflicts++;
        conflictC++;
        if (trailMarker() == 0) return result;
  ...
  int pickBranchConstraint() {
    ...
    for (Clause c : clauses) {
      ...
      assert (propagatedLit != litUndef);
      return propagatedLit;
    }
    return litUndef;
    
  }

  int refute(TIntList out_learnt) {
    for (int i = trail_markers_.size()-1; i > -1; i--) {
      int p = trail_.get(trail_markers_.get(i));
      if (assignment_.get(var(p)) == Boolean.lTrue) {
        out_learnt.add(makeLiteral(var(p), false));
      } else {
        out_learnt.add(makeLiteral(var(p), true));
      }
    }
    return trail_markers_.size()-1;
  }
